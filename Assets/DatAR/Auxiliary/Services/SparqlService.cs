...
public class SparqlService : MonoBehaviour
{
    ...
    public async UniTask<List<DescriptionResource>> GetConceptDescription(string id)
    {
        ...
        var queryResponseRaw = await QueryEndpoint(queryRequest, "https://unity:unity@kgbs-sparql.project.cwi.nl/repositories/KnowledgeGraph-BrainScience");
    ...
    public async UniTask<List<CoordsResource>> GetBrainModelCoordsOfType(string type, string serviceUrl = null, string serviceSubGraphUrl = null)
    {
        serviceSubGraphUrl = serviceSubGraphUrl != null ? $"FROM {serviceSubGraphUrl}" : "";
